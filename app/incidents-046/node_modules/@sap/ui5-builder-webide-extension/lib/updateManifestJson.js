...
function updateManifestFile(workspace, destDir){
	return getManifestFile(workspace).then(function(aManifestFiles){
		if (!!aManifestFiles) {
			var aManifestFilePromises = [];
			aManifestFiles.forEach((oManifestFile) => {
				aManifestFilePromises.push(handleManifest(oManifestFile, workspace, destDir));
				}
			)
			if (!!aManifestFilePromises) {
				return Promise.all(aManifestFilePromises);
			}
		}
	});
}

function handleManifest (oManifestFile, workspace, destDir) {
	if (oManifestFile){
		return oManifestFile.getString().then(function(manifestFileContent) {
			...
			var updatedManifestFile = createJsonFile(JSON.stringify(oManifestJsonContent, null, 4), oManifestFile.getPath());
