    ...
    for (var environment in oEnv) {
        if (oEnv.hasOwnProperty(environment)) {
            if (oEnv[environment] === true || oEnv[environment] === "true") {
                aEnvs[environment] = oEnv[environment];
                ...
                writeInvalidEnv[environment] = oEnv[environment];
            }
        }
    }
    if (Object.keys(aEnvs).length) {
        this.objForCLI.overrideConfig.env = aEnvs;
    }
    return this;
};
    ...
    for (var global in oGlobals) {
        if (oGlobals.hasOwnProperty(global)) {
            if (oGlobals[global] === false) {
                aGlobals[global] = "readonly";
            } else if (oGlobals[global] === true) {
                aGlobals[global] = "writable";
            }
        }
    }
    if (Object.keys(aGlobals).length) {
        this.objForCLI.overrideConfig.globals = aGlobals;
    }
    return this;
};
...
CLIEngineBuilder.prototype.withRules = function (oRules) {
    ...
    this.objForCLI.overrideConfig.rules = oRules;
    return this;
}
...
async function validateFiles(validationMetadata, fileResources) {
    ...
    var cli = new ESLint(cliContainer.objForCLI);
    var paths = [];
    for(const fileResource of fileResources) {
        ...
        if (! await cli.isPathIgnored(relativeToProject)) {
            paths.push(fileResource.getPath());
        }
    }
    const report = await cli.lintFiles(paths);
